<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.37-0ea5e9" alt="当前版本 v1.1.37" />
...
v1.1.37 修复 Team 子号自动化的 Pending invites 误判和人工续跑：只在实际选中的邀请页/邀请记录中核对目标邮箱，不再把 URL 参数或成员页旧 DOM 当成邀请成功；邀请失败后人工完成操作，点击继续会先确认目标状态，再从下一步骤开始，空成员列表不会阻塞已确认的 Pending invite，也不会重复移除成员或发送邀请。内嵌浏览器邀请按钮和页面 Continue 的精确选择、官方 OpenAI PKCE 授权链路、管理员保护和站内接码确认继续保留。
...
v1.1.26 调整 OpenAI OAuth 周额度估算：从本次登录、重新授权或中途接入后首个可读的“百分比 + XIASS 账号已用”基准开始，以累计可读百分点区间的平均成本乘以 100 估算周总额度。每增加完整 1% 才刷新结果，当前百分点内的金额波动不会让显示来回跳动；中途已有非零百分比但本地账号已用为 0 时会先保持“统计中”，直至形成可验证样本；达到 100% 时直接精确显示当前账号已用。账号身份变化、窗口重置、百分比或本地已用回退仍会安全重建采样，不把其他客户端无法归因的消耗混入估算。

用户侧模型价格与原版渠道状态现为两个独立入口：模型价格只展示 XIASS 自定义价格，渠道状态保留原版卡片式稳定性监控，并兼容旧的监控链接。运维首页健康诊断浮层改为高对比度不透明表面，保证深色主题下信息可读。此次补丁继续保留 sub2api v0.1.179 融合后的调用、硬优先级调度、RMB 计费展示、OAuth/接码/令牌转换器、Antigravity/Gemini 兼容、FRP/代理和更新备份能力。上游返回的 Plus/Pro 额度仍以 OpenAI 实际响应为准，XIASS 不会本地制造或提高账号额度。
