<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.7-0ea5e9" alt="当前版本 v1.1.7" />
...
v1.1.7 统一顾客余额邮件的人民币展示：余额不足提醒与充值成功通知不再显示美元符号，官方中英文模板均使用 `¥`；管理员已经保存的旧自定义邮件模板会在读取、预览和再次保存时兼容转换金额占位符，不影响其他仍按美元统计的上游成本与账号用量。保留 XIASS API 品牌、深浅动态主题、模型价格、批量导入、FRP/软路由/SOCKS、在线更新、配置助手、OAuth 授权接码、独立会员接码站、支付回调、iOS 接码流程及全部持久化数据路径；模型广场未合并。
