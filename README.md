<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.6-0ea5e9" alt="当前版本 v1.1.6" />
...
v1.1.6 优化后台操作响应：管理员路由不再等待合规状态网络请求，常用控制台页面共享同一套侧栏、头部与动态主题壳层，切换页面不再重建视频背景和粒子动画；运行概览、账号、分组和风控页面减少重复轮询、重复统计和并发刷新。上游连接补齐 10 秒建连/TLS 握手预算，避免异常 DNS、不可达 IP 或代理链路阻塞故障转移；账号列表窗口成本聚合改为批量查询，主题动态媒体保留播放并缓存复用。保留 XIASS API 品牌、深浅动态主题、模型价格、批量导入、FRP/软路由/SOCKS、在线更新、配置助手、OAuth 授权接码、独立会员接码站、支付回调、iOS 接码流程及全部持久化数据路径；模型广场未合并。
