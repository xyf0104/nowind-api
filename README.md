<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.11-0ea5e9" alt="当前版本 v1.1.11" />
...
v1.1.11 固化 XIASS 账号用量与计费管理能力：账号列表保留中文“请求、账号已用、用户扣费”及账号美元/用户人民币口径，最近 12 小时无有效调用时恢复灰色显示但仍可查看明细；OpenAI OAuth 的 5h 窗口按上游实际返回智能隐藏，7d 周额度随每次额度刷新使用同一份最新响应实时重算。账号扣费明细支持当前窗口、近 30 分钟/1/2/6/24 小时和分钟级自定义时间，能够从账号汇总逐级查看用户与最终上游模型的请求、Token、账号已用和用户扣费。桌面端保持原版 296px 用量布局、查询/次数/重置和刷新链路；手机端周额度强制单行居中，修复时间查询按钮错位，并将账号页及计费详情限制为仅纵向滚动。上述界面、数据口径、API 与交互均纳入 XIASS 二开融合保护。
