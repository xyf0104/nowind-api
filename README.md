<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.0.111-0ea5e9" alt="当前版本 v1.0.111" />
...
v1.0.111 修复 OAuth 授权接码在上游以非 2xx 响应返回单卡第六次上限时被误报为服务不可用的问题：系统现在会准确隔离该卡并自动轮询下一张可用卡。同步优化 XIASS API 全站渲染：仅解码当前主题的视频背景，移除全屏动态高斯模糊并降低粒子背景的像素密度与刷新频率，恢复页面滚动、弹窗和路由切换的流畅度。保留原 OAuth 授权流程、独立授权接码工作台、iOS 管理端接码 UI、XIASS 品牌、深浅主题、模型价格、批量导入、FRP/软路由/SOCKS、在线更新、配置助手及全部持久化数据路径。
