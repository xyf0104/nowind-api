...
v1.1.10 修复 Pixlab 接码状态同步：独立会员接码站顶部“刷新状态”会同步当前号码，OAuth 授权接码与独立接码站均显示按 Pixlab `created_at` 校准的 15 分钟倒计时；超时会原子释放卡密并退回未结算的会员费用，关闭页面后也能在下一次服务端状态读取或领取时自动清理，不再留下无法刷新、取消或重新领取的活跃记录。OpenAI OAuth 周额度会在原版额度刷新后立即使用最新“账号已用 ÷ 7 天使用率”精确重算，例如 `$413.92 ÷ 13%` 显示 `$3,184`。同时修复发布门禁中的数据库游标检查、废弃调度代码与 `nanoid` 安全补丁，保持管理员优先级、XIASS 品牌与动态主题、OAuth 授权接码、独立会员接码站、支付回调、配置助手、iOS 接码流程和全部持久化数据路径不变。
