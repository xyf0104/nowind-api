<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.23-0ea5e9" alt="当前版本 v1.1.23" />
...
v1.1.23 在 XIASS v1.1.21 完整二开基线上完成 sub2api v0.1.179 的针对性融合，并修复融合后由 GitHub 静态检查发现的国产供应商渠道探活适配与代码质量问题。OpenAI 兼容账号继续执行管理员定义的硬优先级：新会话、非粘性请求和普通粘性过期后先选择当前最高优先级可用账号，健康度、负载与 LRU 仅在同一优先级档内参与选择；普通 `session_hash` 保持滚动 1 分钟粘性；不可迁移的 `previous_response_id` 链始终绑定原账号，原账号不可用时返回明确错误，不会静默迁移而破坏会话。此次融合补齐 OpenAI 请求级过载恢复、仅消息型容量错误故障转移、非流式缓冲读取失败重试、WebSocket 后续轮次 429 恢复和 `/responses/input_tokens` 预检，并同步渠道额度监控的账号凭据判定。OpenAI OAuth 周额度约值改用最近完成的 1% 实际额度区间：以达到当前百分点时的最大累计账号已用加上剩余百分点按最近 1% 成本预测；中途登录或重新授权且起始计费为 0 时，用首个完整 1% 补算此前已用部分。模型价格与渠道状态继续分别展示并保持独立开关。上游返回的 Plus/Pro 额度仍以 OpenAI 实际响应为准，XIASS 不会本地制造或提高账号额度。XIASS 品牌、界面、RMB 计费展示、Antigravity/Gemini 模型契约、OAuth/接码/令牌转换器、FRP/代理和更新备份能力均继续保留。

本版同时增强 XIASS 服务端的 OpenAI OAuth 401 自愈：Access Token 尚未达到本地 `expires_at`、但已被上游实际 401 拒绝时，会清除被拒绝令牌并强制进入统一刷新链；管理员连接测试不会再把具备 `refresh_token` 的 OAuth 账号永久置错。Grok 融合则补齐订阅档位与周/月额度识别、按最终模型隔离的额度与粘性状态、上游模型目录观测，以及 CLI/公共/区域端点的管理员默认设置；XIASS 账号显式启用的自定义 URL 和既有安全校验语义保持优先。XIASS Codex 配置助手继续保持原版 `v1.0.97`，本次 XIASS API 发布不改动其鉴权文件和发布产物。
