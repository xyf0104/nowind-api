...
v1.1.22 在 XIASS v1.1.21 完整二开基线上融合 sub2api v0.1.179。OpenAI 兼容账号继续执行管理员定义的硬优先级：新会话、非粘性请求和普通粘性过期后先选择当前最高优先级可用账号，健康度、负载与 LRU 仅在同一优先级档内参与选择；普通 `session_hash` 保持滚动 1 分钟粘性，`previous_response_id` 继续作为不可迁移链路独立保持原账号，但已关闭调度或不可用的账号不会被继续复用。XIASS 品牌、界面、RMB 计费展示、OpenAI 周额度、Antigravity/Gemini 模型契约、OAuth/接码/令牌转换器、FRP/代理和更新备份能力均继续保留。完整的上游功能与修复说明将在融合和回归测试完成后补充。
