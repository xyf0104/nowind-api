<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.9-0ea5e9" alt="当前版本 v1.1.9" />
...
v1.1.9 完善账号用量与调度：账号列表沿用原版布局，将 `A`、`U` 与 `req` 改为“账号已用 $”“用户扣费 ¥”和“次请求”，12 小时内有实际调用时突出显示，无调用时恢复原版灰色；所有账号均可点击用户扣费查看按用户、最终上游模型汇总的请求、Token、账号美元用量与用户人民币扣费，并支持精确到分钟的自定义时间范围以及近 30 分钟、1 小时、2 小时、6 小时和 24 小时快捷筛选。手机端复用相同计费与高亮规则，用量区和计费明细均居中适配窄屏。OpenAI OAuth 额外显示按 7 天实际用量比例估算的周额度，后端未返回或使用率为 0 的 5 小时窗口会自动隐藏。普通请求严格按管理员优先级选择账号，仅在滚动 1 分钟内保留会话粘性，失败、满载或不兼容时再进入下一优先级。OAuth 授权接码与独立会员接码站的手机号仍分开展示区号和本地号码，点击后复制完整国际号码。保留 XIASS API 品牌、动态深浅主题、模型价格、批量导入、FRP/软路由/SOCKS、在线更新、配置助手、OAuth 授权接码、独立会员接码站、支付回调、iOS 接码流程及全部持久化数据路径；模型广场未合并。
