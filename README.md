<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.10-0ea5e9" alt="当前版本 v1.1.10" />
...
v1.1.10 修复 Pixlab 接码状态同步：独立会员接码站顶部“刷新状态”会同步当前号码，OAuth 授权接码与独立接码站均显示按 Pixlab `created_at` 校准的 15 分钟倒计时；当前会话超时会先向 Pixlab 做最后一次查询，后台清理任务也会处理关闭页面后遗留的过期会话，有码则完成，明确无码才原子释放卡密并退回未结算的会员费用，暂时网络失败会保留会话并在下一轮重试。普通状态读取保持只读，不承担结算。OpenAI OAuth 周额度会在原版额度刷新后立即使用同一份最新响应中的“账号已用 ÷ 7 天使用率”精确重算，例如 `$413.92 ÷ 13%` 显示 `$3,184`。同时修复发布门禁中的数据库游标检查、废弃调度代码与 `nanoid` 安全补丁，保持管理员优先级、XIASS 品牌与动态主题、OAuth 授权接码、独立会员接码站、支付回调、配置助手、iOS 接码流程和全部持久化数据路径不变。
