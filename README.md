<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.1.24-0ea5e9" alt="当前版本 v1.1.24" />
...
v1.1.24 修复 OpenAI OAuth 周额度约值在有效 7 天窗口内错误回到“统计中”的问题。上游返回的是随轮询变化的剩余时间，XIASS 现在将其识别为移动 ETA，而不是新的额度窗口；已有可信结果会在当前未完成的百分点内保持展示。每次百分点前进时，系统只使用刚结束百分点内观测到的最大累计账号已用计算最新完整区间成本，并以该闭合端点加剩余百分点预测周额度，不再对历史区间做平滑。中途登录或重新授权、跨百分点缺少中间采样、外部客户端用量以及 100% 用尽均按各自可验证边界处理；已存在的 v1.1.23 采样状态会无损迁移。

用户侧现在只保留 XIASS 模型价格。已移除“可用渠道”和“渠道状态”的菜单、设置卡片及模型价格页内重复入口，旧链接会安全回到模型价格；管理员的渠道监控、渠道定价和原有检测逻辑保持不变。此次补丁继续保留 sub2api v0.1.179 融合后的调用、硬优先级调度、RMB 计费展示、OAuth/接码/令牌转换器、Antigravity/Gemini 兼容、FRP/代理和更新备份能力。上游返回的 Plus/Pro 额度仍以 OpenAI 实际响应为准，XIASS 不会本地制造或提高账号额度。
