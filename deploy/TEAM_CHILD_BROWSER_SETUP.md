...
新创建的地址使用服务器端递增序列：`team1000@<已导入域名>`、`team1001@<已导入域名>`，不会使用前端随机字符串。Redis 部署使用 Redis 原子计数器；单实例无 Redis 的开发部署会把计数写入 `/app/data/team-child-mail-sequence`。两种方式都会跨网页刷新、容器重建和版本更新保留计数。邮箱域名必须来自已上传的邮箱配置；未配置域名时，创建操作会明确提示先补充配置。
...
5. 点击左侧“在内嵌浏览器打开授权”。XIASS 会把官方授权页导航到右侧服务器 Chromium 的当前标签页，不会打开本地浏览器或新建标签。验证码到达后，左侧会自动显示识别出的验证码，可点击复制后粘贴到右侧页面。
...
如果已导入的最新 Team 子号出现 401，账号管理顶部的“创建 Team 子号”按钮会变为淡红色提醒。Team 工作区中的“重新授权”会重新读取当前服务器邮箱会话、清空旧验证码、生成新的 XIASS 官方 PKCE 会话并在配置好的服务器浏览器当前标签页打开授权页；外部页面的账号凭据和多因素验证码仍需在官方页面完成。
