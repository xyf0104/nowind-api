...
import { fetchAllAccountIds, fetchAllAccountSelection } from '../accountSelection'
...
describe('fetchAllAccountIds', () => {
  ...
  it('collects exact platform and account type metadata across every page', async () => {
    const fetchPage = vi.fn(async (page: number) => ({
      items: page === 1
        ...
        : [{ id: 1001, platform: 'openai' as const, type: 'apikey' as const }],
      total: 1001,
      page,
      page_size: 1000,
      pages: 2
    ...
    const snapshot = await fetchAllAccountSelection(fetchPage, { status: 'active' })

    expect(snapshot.ids).toHaveLength(1001)
    expect(snapshot.selectedPlatforms).toEqual(['grok', 'openai'])
    expect(snapshot.selectedTypes).toEqual(['oauth', 'apikey'])
