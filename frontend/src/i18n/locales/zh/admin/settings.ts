export default {
    executionNodes: {
      ...
      pairingTitle: '网页配对',
      pairingDescription: '生成邀请码的节点是本次配对的状态源。目标节点会在确认后通过加密 HTTPS 隧道使用状态源的 PostgreSQL、Redis、JWT 与 TOTP 密钥，并由宿主机安全重建目标应用。',
      ...
      joinHint: '填写状态源 XIASS 的 HTTPS 地址和状态源生成的一次性邀请码。本机将切换到状态源的 PostgreSQL、Redis 和鉴权状态。',
      inviteAuthorityNotice: '生成邀请码不会立即改变流量，但它会把本机确定为对端加入时的状态源。',
      joinAuthorityNotice: '加入会先检查本机没有账号、Key、账单、代理或业务分组，再备份 .env，启动加密状态隧道并只重建本机 XIASS 应用。目标机有业务数据时会拒绝，不会静默覆盖。',
      peerURL: '对端地址',
      ...
      off: '未启用',
      runtimeTitle: '本机运行状态',
      initializeTitle: '初始化本机节点',
      initializeHint: '普通安装无需手动修改环境变量。初始化会创建本机固定出口并安全重建应用，负载均衡开关仍保持关闭。',
      nodeIDPlaceholder: '例如 api2',
      initialize: '初始化节点',
      initializing: '初始化中...',
      initializeAccepted: '本机节点初始化已开始，应用恢复后请刷新本页。',
      initializeFailed: '本机节点初始化失败',
      localNode: '本机节点',
      ...
      issues: {
        ...
        EXECUTION_NODE_RUNTIME_NODE_INVALID: '本机节点 ID 无效，只能使用字母、数字、点、下划线或短横线。',
        EXECUTION_NODE_RUNTIME_APPLIER_UNAVAILABLE: '当前部署没有可用的宿主机配置控制器，请使用 XIASS Docker 安装。',
        EXECUTION_NODE_RUNTIME_PROXY_CONFLICT: '这个节点 ID 已经绑定了其他固定出口，请更换节点 ID 或修复映射。',
        EXECUTION_NODE_RUNTIME_APPLY_FAILED: '宿主机没有成功应用本机节点配置，原配置已尝试恢复。',
        EXECUTION_NODE_WEIGHTS_INVALID: '现有节点权重无效，请先修复后再初始化。',
        EXECUTION_NODE_PAIRING_TARGET_URL_REQUIRED: '配对前请填写目标节点的公网 HTTPS 地址。',
        EXECUTION_NODE_PAIRING_TARGET_URL_INVALID: '目标节点公网地址无效，必须是 HTTPS 地址。',
        EXECUTION_NODE_PAIRING_ROUTING_INVALID: '来源节点无法生成完整的节点权重和固定出口映射。',
    ...
    settings: {
      ...
      scheduling: {
        ...
        executionNodePanelHint: '开关、节点状态、账号权重和固定出口统一在独立负载均衡面板管理，普通设置保存不会覆盖节点配置。',
        openExecutionNodePanel: '打开负载均衡面板',
