export default {
  ...
  usage: {
    ...
    weeklyEstimate: '周额度约',
    weeklyEstimateHint: '首次观察本周用量时只建立基线并显示统计中。形成有效区间后，按“（当前百分点内最大账号已用 - 加入基线账号已用）÷（当前整数百分点 - 基线整数百分点）× 100”估算。例：20% / $0 加入，21% / $20 时约 $2,000；25% / $120 时约 $2,400。同一百分点内采用最大累计值；百分比前进但 XIASS 账号已用未增加时会重建基线。同一 ChatGPT 账号 401 重新授权保留记录，身份或周窗口变化后重新采样，100% 时显示当前实际账号已用。',
