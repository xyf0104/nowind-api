export default {
    executionNodes: {
      ...
      pairingDescription: 'The node that generates the invite becomes the authoritative state source. After confirmation, the target uses the source PostgreSQL, Redis, JWT, and TOTP state through an encrypted HTTPS tunnel and the host safely recreates only the target application.',
      ...
      joinHint: 'Enter the state-source XIASS HTTPS URL and its one-time invite. This node will switch to the source PostgreSQL, Redis, and authentication state.',
      inviteAuthorityNotice: 'Generating an invite does not change traffic immediately, but it establishes this node as the state source for the joining peer.',
      joinAuthorityNotice: 'Joining first checks that this installation has no accounts, keys, billing, proxies, or business groups. It backs up .env, starts an encrypted state tunnel, and recreates only this application. A target with business data is rejected instead of being overwritten.',
      peerURL: 'Peer URL',
      targetNodeID: 'Target node ID',
      targetURL: 'Target public URL',
      targetURLPlaceholder: 'https://api2.example.com',
      ...
      initializeTitle: 'Initialize this node',
      initializeHint: 'A normal Docker installation does not need manual environment edits. This creates the local fixed egress and safely recreates the app; balancing remains disabled until you enable it.',
      ...
      initializeFailed: 'Failed to initialize the local node',
      ...
      issues: {
        ...
        EXECUTION_NODE_RUNTIME_NODE_INVALID: 'The local node ID is invalid. Use only letters, numbers, dots, underscores, or hyphens.',
        EXECUTION_NODE_RUNTIME_APPLIER_UNAVAILABLE: 'This deployment has no host configuration controller. Use the XIASS Docker installation.',
        EXECUTION_NODE_RUNTIME_PROXY_CONFLICT: 'This node ID is already mapped to another fixed egress. Choose another ID or repair the mapping.',
        EXECUTION_NODE_RUNTIME_APPLY_FAILED: 'The host did not apply the local node runtime; the previous configuration was restored if possible.',
        EXECUTION_NODE_WEIGHTS_INVALID: 'The existing node weights are invalid. Repair them before initialization.',
        EXECUTION_NODE_PAIRING_TARGET_URL_REQUIRED: 'Enter the target node public HTTPS URL before pairing.',
        EXECUTION_NODE_PAIRING_TARGET_URL_INVALID: 'The target public URL must be an HTTPS URL.',
        EXECUTION_NODE_PAIRING_ROUTING_INVALID: 'The source node could not create a complete node weight and fixed-egress mapping.',
    ...
    settings: {
      ...
      scheduling: {
        ...
        executionNodePanelHint: 'Manage the switch, node health, account weights, and fixed egress in the dedicated load-balancing panel. Saving general settings does not overwrite node configuration.',
        openExecutionNodePanel: 'Open load-balancing panel',
