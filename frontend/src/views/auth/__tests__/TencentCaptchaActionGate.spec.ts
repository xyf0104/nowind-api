...
function getPasskeyButton(wrapper: ReturnType<typeof mountLogin>) {
  const button = wrapper.findAll('button').find((candidate) =>
    candidate.text().includes('auth.passkeySignIn')
  )
  if (!button) {
    throw new Error('passkey button was not rendered')
  }
  return button
}
...
describe('Tencent captcha action gate', () => {
  ...
  it('passes a fresh Tencent proof to Passkey login', async () => {
    ...
    await getPasskeyButton(wrapper).trigger('click')
    await flushPromises()
