...
func TestMigrateExecutionNodeDefaultWeights_UsesSourceNodeAndMigratesOnce(t *testing.T) {
	...
	mock.ExpectQuery("SELECT value FROM settings WHERE key = \\$1 FOR UPDATE").
		...
		WillReturnRows(sqlmock.NewRows([]string{"value"}).AddRow(`{"api.xiass.com":1,"api2":1}`))
	mock.ExpectExec("UPDATE settings SET value = \\$1, updated_at = NOW\\(\\) WHERE key = \\$2").
		WithArgs(`{"api.xiass.com":9,"api2":1}`, service.SettingKeyExecutionNodeWeights).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("UPDATE settings SET value = \\$1, updated_at = NOW\\(\\) WHERE key = \\$2").
		WithArgs(`{"result":"migrated_9_to_1","source_node_id":"api.xiass.com"}`, service.SettingKeyExecutionNodeDefaultWeightsMigrated).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	repo := &settingRepository{db: db}
	migrated, err := repo.MigrateExecutionNodeDefaultWeights(context.Background(), "api.xiass.com")
	require.NoError(t, err)
...
func TestMigrateExecutionNodeDefaultWeights_PreservesCustomPolicyAndClaimsOnce(t *testing.T) {
	...
	mock.ExpectQuery("SELECT value FROM settings WHERE key = \\$1 FOR UPDATE").
		WithArgs(service.SettingKeyExecutionNodeWeights).
		WillReturnRows(sqlmock.NewRows([]string{"value"}).AddRow(`{"api.xiass.com":3,"api2":1}`))
	mock.ExpectExec("UPDATE settings SET value = \\$1, updated_at = NOW\\(\\) WHERE key = \\$2").
		WithArgs(`{"result":"preserved_custom","source_node_id":"api.xiass.com"}`, service.SettingKeyExecutionNodeDefaultWeightsMigrated).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
...
func TestMigrateExecutionNodeDefaultWeights_DoesNotConsumeMarkerBeforePeerExists(t *testing.T) {
	...
	mock.ExpectQuery("SELECT value FROM settings WHERE key = \\$1 FOR UPDATE").
		WithArgs(service.SettingKeyExecutionNodeWeights).
		WillReturnRows(sqlmock.NewRows([]string{"value"}).AddRow(`{"api.xiass.com":1}`))
	mock.ExpectRollback()
