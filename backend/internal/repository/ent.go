...
// initializeDatabaseWithRetry retries PostgreSQL startup errors only. Invalid
// credentials, migrations, and data errors must fail immediately for operators.
...
func InitEnt(cfg *config.Config) (*ent.Client, *sql.DB, error) {
	...
	// SIMPLE 模式只保留管理员并发设置。分组和价格必须由管理员显式创建，
	// 新安装与后续启动都不能自动补建业务数据。
