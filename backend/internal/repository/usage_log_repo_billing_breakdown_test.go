...
func TestGetAccountBillingUsersUsesCanonicalCostsAndHalfOpenRange(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)COALESCE\(SUM\(`+costExpression+`\), 0\) AS account_cost.*COALESCE\(SUM\(ul\.actual_cost\), 0\) AS user_cost.*WHERE ul\.account_id = \$1 AND ul\.created_at >= \$2 AND ul\.created_at < \$3\s+AND ul\.actual_cost > 0`).
...
func TestGetAccountBillingModelsUsesRequestedModelAndCanonicalCosts(t *testing.T) {
	...
	costExpression := regexp.QuoteMeta(usageLogAccountCostExpression("ul"))
	mock.ExpectQuery(`(?s)`+modelExpression+` AS model.*COALESCE\(SUM\(`+costExpression+`\), 0\) AS account_cost.*COALESCE\(SUM\(ul\.actual_cost\), 0\) AS user_cost.*WHERE ul\.account_id = \$1 AND ul\.user_id = \$2.*ul\.created_at >= \$3 AND ul\.created_at < \$4\s+AND ul\.actual_cost > 0.*GROUP BY `+modelExpression).
