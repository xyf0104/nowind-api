...
func applyMonitorUpdate(existing *ChannelMonitor, p ChannelMonitorUpdateParams) error {
	...
	previousCheckMode := defaultCheckMode(existing.CheckMode)
	if p.Name != nil {
	...
	} else if previousCheckMode == MonitorCheckModeQuota && defaultCheckMode(existing.CheckMode) != MonitorCheckModeQuota && existing.PrimaryModel == MonitorDefaultQuotaModel {
		// 纯 quota 的 "quota" 是持久化占位，不是可请求的模型。切回会发请求的
		// 模式时必须让管理员显式提供真实模型，不能静默把占位值带到上游。
		return ErrChannelMonitorMissingPrimaryModel
	} else if providerChanged && existing.Provider == MonitorProviderGrok && defaultCheckMode(existing.CheckMode) != MonitorCheckModeQuota {
