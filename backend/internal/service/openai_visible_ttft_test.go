...
func TestOpenAIVisibleOutputClassification(t *testing.T) {
	tests := []struct {
		...
		{name: "completed with usage only", data: `{"type":"response.completed","response":{"id":"resp_test","usage":{"input_tokens":1,"output_tokens":2}}}`, want: false},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			require.Equal(t, test.want, openAIStreamDataStartsVisibleOutput(test.data, test.eventType))
		})
	}
}
...
func runSyntheticVisibleTTFTStream(t *testing.T, passthrough bool, visibleDelay time.Duration, timeoutSeconds int, visibleEvent string) *openaiStreamingResult {
	...
	if passthrough {
		...
		passthroughResult, err = service.handleStreamingResponsePassthrough(context.Background(), resp, c, account, started, "test-model", "test-model")
		if passthroughResult != nil {
			result = &openaiStreamingResult{firstTokenMs: passthroughResult.firstTokenMs}
		}
	} else {
		result, err = service.handleStreamingResponse(context.Background(), resp, c, account, started, "test-model", "test-model")
	}
