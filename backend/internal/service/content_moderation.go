...
const (
	...
	// A client can retransmit a policy-rejected request before it receives the
	// completed protocol response. Keep the audit trail, but never let those
	// retransmits send a burst of emails or inflate the auto-ban counter.
	contentModerationDuplicateSideEffectTTL = 10 * time.Minute
)
...
// ContentModerationNotificationDeduper is an optional extension of the hash
// cache. Implementations must reserve a key atomically and expire it after the
// supplied TTL so multiple application instances cannot send duplicate alerts.
//
// It intentionally uses only an opaque digest generated by the service. Raw
// prompt text is never used as a cache key.
type ContentModerationNotificationDeduper interface {
	ReserveFlaggedNotification(ctx context.Context, key string, ttl time.Duration) (bool, error)
}
...
type ContentModerationService struct {
	...
	notificationDedupeMu     sync.Mutex
	notificationDedupe       map[string]time.Time
}
...
func NewContentModerationService(
	...
	svc := &ContentModerationService{
		...
		notificationDedupe:   make(map[string]time.Time),
	}
...
func (s *ContentModerationService) persistContentModerationLog(ctx context.Context, cfg *ContentModerationConfig, log *ContentModerationLog, hashText string, recordHash bool, applySideEffects bool) {
	...
	if applySideEffects && s.reserveFlaggedSideEffects(ctx, log, hashText) {
	...
	} else if applySideEffects {
		slog.Info("content_moderation.duplicate_side_effect_suppressed",
...
// reserveFlaggedSideEffects returns true only for the first matching flagged
// request in a short window. Audit log rows are still written for every
// request; this guard applies solely to notification and auto-ban side effects.
func (s *ContentModerationService) reserveFlaggedSideEffects(ctx context.Context, log *ContentModerationLog, inputHash string) bool {
	key := contentModerationNotificationDedupeKey(log, inputHash)
	if key == "" {
		// Do not accidentally suppress a notification when the request cannot be
		// identified safely (for example, an old log without an input hash).
		return true
	}
	if deduper, ok := s.hashCache.(ContentModerationNotificationDeduper); ok {
		reserved, err := deduper.ReserveFlaggedNotification(ctx, key, contentModerationDuplicateSideEffectTTL)
		if err == nil {
			return reserved
		}
		slog.Warn("content_moderation.notification_dedupe_reserve_failed",
			"user_id", contentModerationEmailUserID(log),
			"endpoint", log.Endpoint,
			"error", err)
	}
	return s.reserveFlaggedSideEffectsLocally(key, time.Now())
}

func (s *ContentModerationService) reserveFlaggedSideEffectsLocally(key string, now time.Time) bool {
	if s == nil || key == "" {
		return true
	}
	s.notificationDedupeMu.Lock()
	defer s.notificationDedupeMu.Unlock()
	if s.notificationDedupe == nil {
		s.notificationDedupe = make(map[string]time.Time)
	}
	for existingKey, expiresAt := range s.notificationDedupe {
		if !expiresAt.After(now) {
			delete(s.notificationDedupe, existingKey)
		}
	}
	if expiresAt, exists := s.notificationDedupe[key]; exists && expiresAt.After(now) {
		return false
	}
	s.notificationDedupe[key] = now.Add(contentModerationDuplicateSideEffectTTL)
	return true
}

func contentModerationNotificationDedupeKey(log *ContentModerationLog, inputHash string) string {
	if log == nil || strings.TrimSpace(inputHash) == "" {
		return ""
	}
	principal := ""
	if log.UserID != nil && *log.UserID > 0 {
		principal = fmt.Sprintf("user:%d", *log.UserID)
	} else if email := strings.ToLower(strings.TrimSpace(log.UserEmail)); email != "" {
...
// contentModerationKeywordRuleIsContextual reports whether a configured rule
// uses the explicit all-terms syntax.
func contentModerationKeywordRuleIsContextual(raw string) bool {
	spec := parseContentModerationKeywordRule(raw)
	return spec.valid && spec.mode == contentModerationKeywordMatchAll
}
...
func matchBlockedKeyword(text string, keywords []string) (string, bool) {
	return newContentModerationKeywordMatcher(keywords).Match(text)
}
