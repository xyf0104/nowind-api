...
type antigravityResponsesStreamAdapter struct {
	anthropicState *apicompat.AnthropicEventToResponsesState
	toolRestorer   *apicompat.ResponsesClientToolStreamRestorer
}
...
func newAntigravityResponsesStreamAdapter(
	...
	if clientToolMap != nil {
		adapter.toolRestorer = apicompat.NewResponsesClientToolStreamRestorer(*clientToolMap)
	}
	return adapter
}
...
func (a *antigravityResponsesStreamAdapter) emitResponseEvent(event apicompat.ResponsesStreamEvent, writer *antigravityClientWriter) {
	events := []apicompat.ResponsesStreamEvent{event}
	if a.toolRestorer != nil {
		events = a.toolRestorer.Restore(event)
	}
...
func (s *AntigravityGatewayService) handleResponsesStreamingFromAntigravity(
	...
	return s.handleResponsesStreamingFromAntigravityWithClientTools(c, resp, startTime, originalModel, nil)
}

func (s *AntigravityGatewayService) handleResponsesStreamingFromAntigravityWithClientTools(
	c *gin.Context,
	resp *http.Response,
	startTime time.Time,
	originalModel string,
	...
	return s.handleAntigravityCompatStream(
		...
		startTime,
		originalModel,
		newAntigravityResponsesStreamAdapter(originalModel, clientToolMap),
