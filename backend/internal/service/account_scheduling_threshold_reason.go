...
func BuildTempUnschedReasonPayload(source, errorMessage string) string {
...
func BuildAccountSchedulingThresholdReason(errorMessage string) string {
	return BuildTempUnschedReasonPayload(AccountSchedulingThresholdReasonSource,
...
func parseTempUnschedReasonPayload(rawReason string) (tempUnschedReasonPayload, bool) {
	...
	if json.Unmarshal([]byte(rawReason), &payload) != nil {
		return tempUnschedReasonPayload{}, false
	}
...
func normalizeTempUnschedReasonErrorMessage(errorMessage, fallback string) string {
	if value := strings.TrimSpace(errorMessage); value != "" {
		return value
	}
...
func buildAccountSchedulingThresholdErrorMessage(input AccountSchedulingThresholdReasonInput) string {
	...
	return fmt.Sprintf("%s scheduling threshold reached for %s: %.1f%% used >= %d%%; paused until %s",
		platform, target, input.UsedPercent, threshold, untilText)
}

func tempUnschedStateFromStoredReason(rawReason string, fallbackUntilUnix int64) *TempUnschedState {
	state := &TempUnschedState{UntilUnix: fallbackUntilUnix, RuleIndex: -1}
	rawReason = strings.TrimSpace(rawReason)
	if rawReason == "" {
	...
	if json.Unmarshal([]byte(rawReason), &parsed) == nil {
