...
func TestApplyOpenCodeSessionHeaderTrustBoundary(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "official origin with explicit TLS port",
			account:   openCodeSessionTestAccount("https://opencode.ai:443/zen/v1"),
			targetURL: "https://opencode.ai:443/zen/v1/responses",
			incoming:  "conversation-123",
			want:      "conversation-123",
		},
		{
		...
		},
		{
			...
			account:   openCodeSessionTestAccount("https://notopencode.ai/v1"),
			targetURL: "https://notopencode.ai/v1/responses",
			incoming:  "conversation-123",
		},
		{
...
func TestApplyOpenCodeSessionHeaderReplacesEveryOverrideCasing(t *testing.T) {
	gin.SetMode(gin.TestMode)
	headers := http.Header{
		"x-opencode-session": {"fixed-lower"},
		"X-Opencode-Session": {"fixed-canonical"},
		"X-OpenCode-Session": {"fixed-wire"},
	}
	applyOpenCodeSessionHeader(
		newOpenCodeSessionTestContext(t, "conversation-override"),
		openCodeSessionTestAccount("https://opencode.ai/zen/v1"),
		"https://opencode.ai/zen/v1/responses",
		headers,
	)
	requireSingleOpenCodeSessionHeader(t, headers, "conversation-override")
}
...
func TestOpenCodeSessionIsNotForwardedToOtherUpstreams(t *testing.T) {
	...
	for _, baseURL := range []string{
		...
		t.Run(baseURL, func(t *testing.T) {
			...
			t.Run("normal responses", func(t *testing.T) {
			...
			t.Run("passthrough responses", func(t *testing.T) {
				req, err := svc.buildUpstreamRequestOpenAIPassthrough(context.Background(), c, account, body, "token")
				require.NoError(t, err)
				require.Empty(t, req.Header.Get(openCodeSessionHeader))
			})
			t.Run("raw chat completions", func(t *testing.T) {
				upstream := &openCodeSessionHTTPUpstream{}
				svc.httpUpstream = upstream
				targetURL, err := svc.openAIChatCompletionsTargetURL(account)
				require.NoError(t, err)
				resp, err := svc.sendCCUpstreamRequest(
					context.Background(), c, account, targetURL,
					[]byte(`{"model":"gpt-5"}`), false, "token", "", "",
				)
				require.NoError(t, err)
				require.NoError(t, resp.Body.Close())
				require.NotNil(t, upstream.request)
				require.Empty(t, upstream.request.Header.Get(openCodeSessionHeader))
			})
		})
	}
}
